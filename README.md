...
- `codex-coms status` shows local state, including sidecar timing and pending wake-command work when available.
...
- `codex-coms wake trigger --json` starts the configured local wake command for the next pending wake event that has not already attempted one. Pass `--retry-attempted` to retry an attempted pending event after fixing local wake configuration.
...
- `src/wake/codexWake.ts`: optional wake helper.
  ...
  - `readWakeCommandStatus`: reports pending wake events, pending unattempted wake-command events, and wake handler liveness.
  ...
  - `triggerPendingWakeCommand`: starts the configured local wake command for the next pending unattempted event, or explicitly retries an attempted pending event.
